...
const Explore: React.FC<ExploreProps> = ({
  ...
  if (mounted.length > 1) {
    umountAll();
    showErrorMessage('Unexpected Error', 'Multiple repos have been mounted somehow so all repos have been unmounted.')
    return (<></>);
  }

  const { projectRepos, selectedRepo, branches, selectedBranch } = getMountedStatus(mounted, unmounted);
    ...
    <div className="pachyderm-explore-view">
      ...
      <DropdownCombobox
        ...
        onSelectedItemChange={(selectedBranch) => {
          if (!selectedRepo) {
            umountAll();
            showErrorMessage('Unexpected Error', 'An unknown branch was selected so all repos have been unmounted.')
            return
          }
...
const mount = async (projectRepo: string, branch: string = 'master'): Promise<ListMountsResponse> => {
...
const getMountedStatus = (mounted: Mount[], unmounted: Repo[]): MountedStatus => {
